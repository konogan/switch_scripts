...
async function jobArrived(s, flowElement, job) {
    try {
        ...
        const preflight_report = path_1.default.join(jobPath, job.getName() + ".xml");
        const preflight_preview = path_1.default.join(jobPath, job.getName() + ".jpg");
        ...
        const preflight_report_out = tmp_1.default.fileSync();
...
async function generate_report(preflight, vignette, report, job) {
    let xml = await fs_extra_1.default.readFile(preflight);
    let jsonreport = await xml2js_1.default
    ...
    function drawLocation(loc, color) {
        ...
        doc
            .rect(m2p(10) + m2p(rx), m2p(10) + m2p(ry), m2p(rw), m2p(rh))
            .lineWidth(0.5)
            .fillOpacity(0.5)
            .fillAndStroke(color, color);
    }
    ...
    doc
        ...
        .stroke("black");
    doc
        .rect(m2p(10) + m2p(bx * ratio), m2p(10) + m2p(by * ratio), m2p(bw * ratio), m2p(bh * ratio))
        .lineWidth(1)
        .stroke("blue");
    doc
        .rect(m2p(10) + m2p(tx * ratio), m2p(10) + m2p(ty * ratio), m2p(tw * ratio), m2p(th * ratio))
        .lineWidth(1)
        .stroke("green");
    //   doc
    ...
    let content = await get_stream_1.default.buffer(doc);
    await job.log(LogLevel.Info, "Save report");
    await fs_extra_1.default.writeFile(report, content);
}
